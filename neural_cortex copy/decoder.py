...
def Er_per_sample(glm_ctx, S, cx, cy, spikes_on_t, spikes_off_t, gx_w, gy_w):
    lam_on, lam_off = 
    dt = glm_ctx.dt
    return ((lam_on * dt - spikes_on_t * torch.log(lam_on * dt + glm_ctx.t_eps)).sum(dim=1)
            + (lam_off * dt - spikes_off_t * torch.log(lam_off * dt + glm_ctx.t_eps)).sum(dim=1))


def Er(glm_ctx, S, samples_t, spikes_on_t, spikes_off_t, device, gx_w=None, gy_w=None):
    ...
    per_sample = Er_per_sample(glm_ctx, S, cx, cy, spikes_on_t, spikes_off_t, gx_w, gy_w)
...
def adam_update_A(glm_ctx, A_param, Hessian, optimizer,
                  samples_t, spikes_on_t, spikes_off_t,
    ...
    for _ in range(n_iter):
        ...
        Er_val = Er(glm_ctx, S, samples_t, spikes_on_t, spikes_off_t, device, gx_w, gy_w)
...
def compute_gauss_newton_hessian(glm_ctx, A, samples_t, spikes_on_t, spikes_off_t,
    ...
    for i in range(n_samples):
        ...
        gx_i = gx_w[i:i+1]
        gy_i = gy_w[i:i+1]
        r_on_i = spikes_on_t.unsqueeze(0)
        r_off_i = spikes_off_t.unsqueeze(0)
...
def update_hessian(glm_ctx, H_prev, A_anchor, samples_t,
                   spikes_on_t, spikes_off_t, tau, dt,
    ...
    H_new = compute_gauss_newton_hessian(
        glm_ctx, A_anchor, samples_t, spikes_on_t, spikes_off_t,
        gx_w, gy_w, img_size, mean_offset, device,
    )
